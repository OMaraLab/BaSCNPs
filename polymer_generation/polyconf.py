        ...
        #        done = clash >= cutoff
            ...
            while not done:
                ...
                if tries >= 10 and not first:
                    conf,clash7 = shuffle(conf,sel=f"(resid {prev} and name CA) or (resid {prev} and name C)", mult=mult*2) # try shuffling the -C CA bond too
                    conf,clash8 = shuffle(conf,sel=f"(resid {prev} and name C) or (resid {i} and name CA)", mult=mult*2) # try shuffling the -C CA bond too
